        ...
        loader: {
            ...
            _loadSrc: function _loadSrc(src, callback) {
                ...
                foreach(all("script"), function(script) {
                    var src = script.src;
                    if (src && !loaded[src]) loaded[src] = true;
                });
                if (loaded[script.src]) {
                    if (callback) loaded[script.src].push(callback);
                }
                else {
                    listenOnce(script, "load", "onreadystatechange", function() {
                        if (loaded[script.src] instanceof Array) {
                            foreach(loaded[script.src], function(callback) {callback();});
                        }
                    }, 
                    true, true);

                    this._loading++;
                    loaded[script.src] = [callback];
