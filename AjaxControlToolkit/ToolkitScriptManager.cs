...
namespace AjaxControlToolkit
{
    ...
    [Themeable(true)]
    public class RadToolkitScriptManager : ScriptManager
    {
        ...
        static RadToolkitScriptManager()
        {
        ...
        /// </summary>
        /// <remarks>
        /// The handler's ProcessRequest method should call directly through to RadToolkitScriptManager.OutputCombinedScriptFile
        /// </remarks>
        [UrlProperty]
        ...
        private void ApplyAssembly(ScriptReference script, bool isComposite)
        {
                ...
                (String.IsNullOrEmpty(script.Assembly) || Assembly.Load(script.Assembly) == typeof(ScriptManager).Assembly))
            {
                ...
                {
                    script.Assembly = typeof(RadToolkitScriptManager).Assembly.FullName;
                }
            }
        }
#endif
        ...
        protected override void OnResolveScriptReference(ScriptReferenceEventArgs e)
        {
            ...
            if (CombineScripts && !String.IsNullOrEmpty(e.Script.Assembly) && !String.IsNullOrEmpty(e.Script.Name))
            {
                // Initialize
                ...
                if (IsScriptCombinable(scriptEntry))
                {
                    ...
                    if (null == _combinedScriptUrl)
                    {
                        ...
                        _combinedScriptUrl = String.Format(CultureInfo.InvariantCulture, "{0}?{1}={2}&{3}={4}", ((null != CombineScriptsHandlerUrl) ? CombineScriptsHandlerUrl.ToString() : Page.Request.Path.Replace(" ", "%20")), HiddenFieldParamName, HiddenFieldName, CombinedScriptsParamName, HttpUtility.UrlEncode(SerializeScriptEntries(_scriptEntries, false)));
                    }
        ...
        public static bool OutputCombinedScriptFile(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(hiddenFieldName) && !string.IsNullOrEmpty(combinedScripts))
            {
                ...
                using (StreamWriter outputWriter = new StreamWriter(outputStream))
                {
                    ...
                    AntiXssWriter(scriptEntries, outputWriter, hiddenFieldName, 3);
                }
        ...
        private static void AntiXssWriter(List<ScriptEntry> scriptEntries, StreamWriter outputWriter, string data, int typeWriter = 1)
        {
            ...
            switch (typeWriter)
            {
                ...
                case 3:
                    outputWriter.WriteLine(string.Format(CultureInfo.InvariantCulture,
                        ...
                        "Sys.Application.add_load(fn);" +
                        "}})();",
                        System.Web.Security.AntiXss.AntiXssEncoder.HtmlEncode(data,false),
                        SerializeScriptEntries(scriptEntries, true)));
                    break;
                ...
                case 4:
                    outputWriter.WriteLine(string.Format(CultureInfo.InvariantCulture,
                        ...
                        "Sys.Application.add_load(fn);" +
                        "}})();",
                        System.Web.HttpUtility.HtmlEncode(QuoteString(data)),
                        SerializeScriptEntries(scriptEntries, true)));
                    break;
                ...
                case 9:
                    outputWriter.WriteLine(string.Format(CultureInfo.InvariantCulture,
                        ...
                        "Sys.Application.add_load(fn);" +
                        "}})();",
                        QuoteString(new HtmlSanitizer().Sanitize(data)),
                        SerializeScriptEntries(scriptEntries, true)));
                    break;
            }             
        }
            ...
            foreach (ScriptEntry scriptEntry in scriptEntries)
            {
                if (!scriptEntry.Loaded)
                {
                    ...
                    if (WebResourceRegex.IsMatch(script))
                    {
                        ...
                        throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "RadToolkitScriptManager does not support <%= WebResource/ScriptResource(...) %> substitution as used by script file \"{0}\".", scriptEntry.Name));
                    }
...
#pragma warning restore 0618
        ...
        private class RedirectScriptReference : ScriptReference
        {
            ...
            {
                Name = name;
                Assembly = typeof(RadToolkitScriptManager).Assembly.FullName;
            }
