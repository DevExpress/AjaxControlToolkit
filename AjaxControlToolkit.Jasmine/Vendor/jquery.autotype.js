...
(function($) {
    ...
    var CHARACTER = 1,
        ...
        parseCodes = function(value, codeMap) {
            ...
            for(var i = 0; i < value.length; i++) {
                if(!definingControlKey && i <= value.length - 5 &&
                    value.charAt(i) === '{' && value.charAt(i + 1) === '{') {
                    i++;

                    definingControlKey = true;
                } else if(definingControlKey && i <= value.length - 2 &&
                    value.charAt(i) === '}' && value.charAt(i + 1) === '}') {
                    // skip the next "}"
                    i++;
                    ...
                    if((!isNullOrEmpty(previousChar) && areDifferentlyCased(previousChar, character)) ||
        ...
        triggerCodeOnField = function(code, field) {
            ...
            var keyDownEvent = $.extend($.Event(), evnt, {
                type: 'keydown', keyCode: code.keyCode, charCode: 0, which: code.keyCode
            });
            var keyPressEvent = $.extend($.Event(), evnt, {
                type: 'keypress', keyCode: 0, charCode: code.charCode, which: code.charCode || code.keyCode
            });
            var keyUpEvent = $.extend($.Event(), evnt, {
                type: 'keyup', keyCode: code.keyCode, charCode: 0, which: code.keyCode
            });
    ...
    $.fn.autotype = function(value, options) {
        ...
        var settings = $.extend({}, $.fn.autotype.defaults, options),
            codes = parseCodes(value, settings.keyCodes[settings.keyBoard]);
