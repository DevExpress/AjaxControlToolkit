...
$type.prototype = {
    ...
    addLink: function DataContext$addLink(sourceEntity, sourcePropertyName, targetEntity) {
        ...
        // Example:
        //      addLink(image, "tags", tag)
        // This will add the tag to the image.tags array, creating the array if it doesnt already exist.
        // To set a one-to-X relationship use setLink.
        ...
        if (!toggled || (toggled.action !== Sys.Data.ChangeOperationType.remove)) {
            // register a new change if we did not just cancel out a previous remove
            // but not if we attempted to insert a link that already existed, unless thats because it
            // was inserted already by the toggled insert change.
    ...
    removeLink: function DataContext$removeLink(sourceEntity, sourcePropertyName, targetEntity) {
        ...
        if (!toggled || (toggled.action !== Sys.Data.ChangeOperationType.insert)) {
            // register a new change if we did not just cancel out a previous insert
            // but not if we attempted to remove a link that didnt already exist, unless thats becuase it was
            // removed already by the toggled removal change.
    ...
    setLink: function DataContext$setLink(sourceEntity, sourcePropertyName, targetEntity) {
        ...
        // Example:
        //      setLink(image, "contributor", person)
        // This will set the image.contributor field to the person
        // To add to a many-to-X relationship to null use addLink.
    ...
    fetchDeferredProperty: function DataContext$fetchDeferredProperty(entity, propertyName, parameters, mergeOption, succeededCallback, failedCallback, timeout, userContext) {
        ...
        if (query && query.operation) {
            function done(result) {
                // null merge option passed to setField() since we want to force setting the field no matter what
                _this._setField(entity, propertyName, null, result, null, true);
                // the entity or entities will already be stored since fetchData takes care of that.
    ...
    removeEntity: function DataContext$removeEntity(entity) {
        ...
        // if it was added and now removed, just remove it from the inserts
        if (this._peekChange(this._inserts, entity, identity, true)) {
            // was an inserted item that is now being removed. We just removed it
            // from the inserts collection, now remove it from the changelist
            // and remove any links added or set on it or to it.
            this._removeChanges(entity, "*");
            unregister();
        }
        else {
            // it was an existing item...
            // if existing item was edited and then removed, it should just be removed
            this._deletes = this._pushChange(this._deletes, entity, identity);
            if (this._peekChange(this._edits, entity, identity, true)) {
                unregister();
            }
            // remove any links added to it but not removed or set 
            // scenario: deleting a link from an entity or to an entity, then removing that entity
            // e.g.:
            // removeLink(product, "categories", sports); // stays in changeset
            // setLink(product, "createdBy", null); // stays in changeset
            // addLink(product, "categories", bikes); // should be removed from changeset
            // addLink(bikes, "products", product); // should be removed from changeset
        ...
        function done(data) {
            ...
            if (succeededCallback) {
                if ((data instanceof Array) && (trackedData === data)) {
                    // clone array so that the client has their own copy of the array and can insert
                    // and remove items without affecting the data context. If they want to insert or remove
                    // and item, they should call insertEntity or removeEntity.
    ...
    _fixAfterSave: function DataContext$_fixAfterSave(change, entity, result) {
        // the response may have changed fields, so update any fields in the response on the original object
        // mergeOption for after save is always overwriteChanges so we just omit it
        if (this._useIdentity) {
            var oldIdentity = this.getIdentity(entity),
                newIdentity = this.getIdentity(result);
            this._combine(entity, result);
            if (oldIdentity !== newIdentity) {
                // inserted items have their identities change when successful inserted, because
                // the server responses with the actual identity.
                // so we need to find the old identity and change it to the new identity
    ...
    trackData: function DataContext$trackData(data, mergeOption) {
        ...
        // also called after results of a query return.
        if (this._useIdentity) {
            if ((typeof(mergeOption) === "undefined") || (mergeOption === null)) {
                mergeOption = this.get_mergeOption();
            }
            ...
            if (data instanceof Array) {
                ...
                if (trackedData.length === 0) {
                    // the single item was a deleted item
                    data = null;
                }
            }
            // get_items has all the (unordered) data
            // do not 'release' the old data since they are still managed in the items dictionary.
            var oldData = this._lastResults;
            ...
            // without identity management, throw away all the old data and changes first
    ...
    _processResults: function DataContext$_processResults(dataContext, changes, results) {
        if (results && results.length === changes.length) {
            // as we update the original items we dont want to consider them edits
    ...
    saveChanges: function DataContext$saveChanges(succeededCallback, failedCallback, userContext) {
        ...
        function done(results) {
            if (_this._disposed) return;
            if (!delay) {
                // done() was called synchronously, probably because there were no items to update,
                // or the saveChangesMethod is synchronous.
                // To maintain consistent behavior when there are items to commit, we ensure this
                // method behaves asynchronously using a setTimeout.
                delay = true;
                window.setTimeout(function() { done(results) }, 0);
            }
            else {
                // scan results for updated data
                _this.clearChanges();
        ...
        // sometimes there was a change but then it was reverted, like if they
        // added an item then removed it.
        if (changes.length === 0) {
            done(null);
            return null;
        }
    ...
    _removeChanges: function DataContext$_removeChanges(entity, linkField, addsOnly) {
        // removes changeset items for the given entity.
        // linkField set means to remove links for the given field (e.g. adds/removes to product.categories)
        // linkField may be set to "*" to mean any field, in which case links TO this entity are removed as well
        // addsOnly true means for links, only remove additions, not removals or updates (scenario: removing an existing entity)
        ...
        for (i = 0, l = changes.length; i < l; i++) {
            ...
            if (// doing links and the entity is the source, and the linkField is correct
                ...
                // doing entities and the entity is the changed item
    ...
    _toggleLink: function DataContext$_toggleLink(source, field, target, removing) {
        ...
        for (var i = 0, l = changes.length; i < l; i++) {
            change = changes[i];
            // when removing a link, find any previous inserts of this exact combination and remove it,
            // or find any 'update' where we dont care what the target is, because we are going to reset it to null.
            // when inserting a link, find any removals of this exact combination and remove it,
            // or find any 'update' where we dont care what the target is, because we are going to set
    ...
    _capture: function DataContext$_capture(data) {
        // begins listening to collection change and/or property changed events on the new data items
        if (data instanceof Array) {
    ...
    _dataChanged: function DataContext$_dataChanged(object, args) {
        // _ignoreChange: changed because we are merging with an incoming entity, not an edit
        if (this._ignoreChange) return;
        var changelist = this.get_changes();
        var identity = this.getIdentity(object);
        if (!this._peekChange(this._inserts, object, identity)) {
            // item is updating but it may be a new item anyway. we know it isnt now
            var alreadyChanged = this._peekChange(this._edits, object, identity);
            if (!alreadyChanged) {
                // this isnt an edit for an already edited item
    ...
    _release: function DataContext$_release(data) {
        // begins listening to collection change and/or property changed events on the new data
        if (data instanceof Array) {
            for (var i = 0, l = data.length; i < l; i++) {
    ...
    _filterLinks: function DataContext$_filterLinks(changeSet) {
        // returns a modified changeset that contains the changed entities without any links to other entites,
        // unless there's no identity management.
    ...
    _getValueType: function DataContext$_getValueType(parent, name, object) {
        ...
        // 2 = value set (null or any other value)
        var type = typeof(object);
        if (type === "undefined") return 0;
        if ((object === null) || (type !== "object")) return 2;
        // we know it is an object, but is it a deferred object?
    ...
    _setField: function DataContext$_setField(target, name, source, value, mergeOption, isLink) {
        ...
        if (!isArray) {
            // dont set it if theres already a value (including null),
            // unless that value is an Object and it is a deferred placeholder
            var targetField = target[name], valueType = this._getValueType(target, name, targetField);
            // appendOnly: only overwrite 0 and 1 types
            // overwriteChanges: overwrite 0 and 1, and 2 if source is not a 1
            if (appendOnly) {
                // Dev10 646486: Even in appendOnly mode, we set the field if they have the same identity,
                // which is necessary for identity management (they have the same identity, should always be
                // the same object). This case also covers simple values so we must also doSet=false if they arent
                ...
                // dont overwrite non-deferred entity with a deferred entity
                doSet = false;
            }
        }
        if (doSet) {
            if (isArray) {
                // note: we dont actually want this to be an observable operation.
                // this only occurs when updating an existing array with a source array, which only
                // occurs when registering incomming data items, which the client is already expecting to
                // result in modified references. We never merge two arrays otherwise.
                target[name] = value;
            }
            else {
                this._ignoreChange = true;
    ...
    _combine: function DataContext$_combine(target, source, mergeOption, excludeEntities) {
        // merges target and source's fields, processing any entities found in the source
        // note: entities cannot contain arrays that aren't other entities.
        // Entities can have complex object fields (e.g. Address) but that they cannot subsequently contain entities.
        // Therefore: any array found is always sub-entities, and any object found is an entity if it has metadata or
        // is a deferred reference.
        // NOTE: if target is not specified, this was called just so we can find entities
        // on the source.
        ...
        for (var name in source) {
            ...
            if (this._useIdentity && (field instanceof Array)) {
                if (!excludeEntities) {
                    // array should replace target array but its values are processed
                ...
                if (identity !== null) {
                    if (!excludeEntities) {
                        // source entity contains another entity. Make sure it is resolved
                        // and then assign it to the target.
                        this._storeEntity(identity, field, target, name, source, mergeOption);
                    }
                }
                else if (target) {
                    // not an entity, but it could be a deferred entity (or we arent doing identity management).
                    // Don't set it if the target already has an entity in this place
                    var targetField = target[name];
                    if (targetField && (typeof(targetField) === "object") && this.getIdentity(targetField)) {
                        // dont write a non-entity field onto an object that already has a filled entity
                        continue;
                    }
                    if (this._setField(target, name, source, field, mergeOption) && !removedChanges &&
                        ...
                        // just set a field on the parent, revert any update changes in the changeset for it
                        removedChanges = true;
    ...
    _storeEntity: function DataContext$_storeEntity(identity, entity, parent, parentField, source, mergeOption) {
        ...
        if ((typeof (storedEntity) !== "undefined")) {
            if (storedEntity === entity) {
                ...
                // collision -- an entity given already exists, update given fields on the stored entity
                this._combine(storedEntity, entity, mergeOption);
            }
        }
        else {
            // a new entity was found
            // make sure it is not logged as a deleted entity
            this._items[identity] = storedEntity = entity;
            // watch entity for changes
            this._captureEntity(entity);
            // look for entities within the entity's fields
            // Dev10 646486: The entity may have links to other entities, so we call _combine
            // as usual. But those entities might already exist, so they will be identity managed,
            // in which case the entity's field needs updating. So we pass the entity as both the
            // source and target in this case. Previously we passed null for the first parameter,
            // because it was assumed since this is a new entity it wont need any fields updated.
    ...
    _storeEntities: function DataContext$_storeEntities(entities, mergeOption) {
        ...
        for (i = 0, l = entities.length; i < l; i++) {
            ...
            if (isObject) {
                if (appendOnly) {
                    // when append only, deleted entities shouldnt reappear
                ...
                if (identity !== null) {
                    if (this._storeEntity(identity, entity, entities, i, null, mergeOption) && !appendOnly) {
                        // when overwrite changes, logged deletions should be cleared
                        this._removeChanges(entity);
                    }
                }
            }
        ...
        if (deleted) {
            // return a filtered array that does not contain deleted entities
            // we create this filtered list after enumerating all the entities so
            // that any stored entities that existed already are correctly referenced
            // since the entries in the entities array may have been altered by _storeEntity.
