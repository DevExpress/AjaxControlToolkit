		...
		public string Value
		{
			get
			{
				if (_value == null)
				{
					...
					//if this is the value of "Style", we need to parse and sort as well
					if (Name.Equals("style", StringComparison.OrdinalIgnoreCase)) {
						...
						Array.Sort(values);
						_value = String.Join("; ", values, 0, values.Length);
					}
					//IE8 sometimes puts whitespaces between coords eles
					else if(Name.Equals("coords", StringComparison.OrdinalIgnoreCase)) {
						_value = _value.Replace(" ", String.Empty);
					}
					//IE8 and IE7 have difference casing for shape node
