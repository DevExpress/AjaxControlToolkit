...
namespace HtmlAgilityPack
{
	...
	public class HtmlNode: IXPathNavigable
	{
		...
		internal void AspNetPreProcessingNode()
		{
			// AspNetCustomizedCode
			// Adding type attribute if input node had no type attributed defined already
			if (Name.Equals("input", StringComparison.InvariantCultureIgnoreCase) && Attributes.GetAttributeIndex("type") < 0) {
				Attributes.Append("type", "text");
			}
			// Removing "colSpan=0" from td node
			if (Name.Equals("td", StringComparison.InvariantCultureIgnoreCase)) {
				int colSpanIndex = Attributes.GetAttributeIndex("colSpan");
				if (colSpanIndex>=0 && Attributes[colSpanIndex].Value.Equals("0")) {
					Attributes.RemoveAt(colSpanIndex);
				}
			}
		}
		...
		internal static void WriteAttributes(XmlWriter writer, HtmlNode node)
		{
			...
			// AspNetCustomizedCode: sorted by attribute name
			foreach (HtmlAttribute att in node.Attributes._hashitems.Values.Cast<HtmlAttribute>().OrderBy(a => a.XmlName))
			{
		...
		internal void WriteAttributes(TextWriter outText, bool closing)
		{
			if (_ownerdocument.OptionOutputAsXml)
			{
				...
				// we use _hashitems to make sure attributes are written only once
				// AspNetCustomizedCode: sorted by attribute name
				foreach(HtmlAttribute att in _attributes._hashitems.Values.Cast<HtmlAttribute>().OrderBy(a => a.XmlName))
				{
			...
			if (!closing)
			{
				...
				{
					// AspNetCustomizedCode: sorted by attribute name
					foreach (HtmlAttribute att in _attributes.Cast<HtmlAttribute>().OrderBy(a => a.XmlName))
					{
						WriteAttribute(outText, att);
					}
				}
			...
			else
			{
				...
				// AspNetCustomizedCode: sorted by attribute name
				foreach (HtmlAttribute att in _endnode._attributes.Cast<HtmlAttribute>().OrderBy(a => a.XmlName))
				{
					WriteAttribute(outText, att);
				}
				if (_ownerdocument.OptionAddDebuggingAttributes)
