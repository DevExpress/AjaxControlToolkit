...
namespace AjaxControlToolkit.Tests {
    ...
    public class AjaxFileUploadTests {
        ...
        [Test]
        public void FileWithoutExtensionIsAccepted() {
            var request = new WorkerRequest(testBody, testQueryNoExtension, testContentType);
            var context = new HttpContext(request);
            AjaxFileUploadHelper.Process(context);
            Assert.True(File.Exists(Path.Combine(_tempFolder, "E63F2078-D5C7-66FA-5CAD-02C169149BD5", "aaa.tmp")));
        }
